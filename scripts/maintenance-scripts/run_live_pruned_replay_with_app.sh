...
# test scenario:
# 1. HAF is replayed to 1m of blocks and stops, testing the initial pruned replay behavior.
# 2. HAF is started in the background to continue replay with limit 1.02m and a huge psql-live-synch-threshold.
...
# 4. HAF app is stopped after successfully syncing 1.02m of blocks.
#
...
export REPLAY=("--replay-blockchain" "--psql-prune-blocks=10" "--stop-at-block=$NUMBER_OF_BLOCKS_TO_FIRST_REPLAY")
export REPLAY_CONTINUATION=("--replay-blockchain" "--psql-prune-blocks=10" "--psql-livesync-threshold=1000000000" "--stop-at-block=$LAST_BLOCK_TO_SYNC")
