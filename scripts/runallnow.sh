...
#    ninja extension.hive_fork_manager  \
    ...
    ninja  query_supervisor hived extension.hive_fork_manager && sudo ninja install && sudo chown $USER:$USER .ninja_* && ctest -R keyauth --output-on-failure # && ctest -R curr         
    EXIT_STATUS=$?
...
app_start()
{
    ...
    psql -a -v "ON_ERROR_STOP=1" -d haf_block_log -c '\timing'  -c "call cab_app.main('cabc', $RUN_APP_MAIN_TILL_BLOCK, $RUN_APP_MAIN_CHUNK_SIZE, '$CONSENSUS_STORAGE')" -c 'select * from hive.cabc_current_account_balance_state_provider LIMIT 30;' -c 'select count(*) from hive.cabc_accounts;' 2>&1 | tee -i app.log # run
}
...
app_cont()
{
    ...
    -c 'select * from hive.cabc_current_account_balance_state_provider limit 30;' -c 'select count(*) from hive.cabc_accounts;' \
    -c 'select SUM(balance) from hive.cabc_current_account_balance_state_provider' \
    2>&1 | tee -i app.log # run
    echo "After app_cont"

# Compare if returned 15 top accounts are equal to the pattern
    PSQL_RESULT=$(psql -t -d haf_block_log  -c "(SELECT account, balance, ROW_NUMBER() OVER (ORDER BY balance DESC)  FROM hive.cabc_current_account_balance_state_provider LIMIT 15)
    EXCEPT
