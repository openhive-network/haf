...
$BODY$
BEGIN
    CALL hive.dlogs('<no-context>', 'Entering revert_insert');
    EXECUTE format(
    ...
    CALL hive.dlogs('<no-context>', 'Exiting revert_insert');
...
$BODY$
BEGIN
    CALL hive.dlogs('<no-context>', 'Entering revert_delete');
    EXECUTE format(
        'INSERT INTO %I.%I( %s )
        (
            SELECT %s
    ...
    CALL hive.dlogs('<no-context>', 'Exiting revert_delete');
...
$BODY$
BEGIN
CALL hive.dlogs('<no-context>', 'Entering revert_update');
...
EXECUTE format(
    ...
    CALL hive.dlogs('<no-context>', 'Exiting revert_update');
...
$BODY$
BEGIN
    CALL hive.dlogs('<no-context>', 'Entering back_from_fork_one_table');
    EXECUTE format(
        'SELECT
    ...
    CALL hive.dlogs('<no-context>', 'Exiting back_from_fork_one_table');
