...
BEGIN
    CALL hive.dlogs(_context_id, 'Entering create_triggers');
    ...
    CALL hive.dlogs(_context_id, 'Exiting create_triggers');
...
DECLARE
    ...
    __hive_truncate_trigger_name TEXT := hive.get_trigger_truncate_name( _table_schema,  _table_name );
BEGIN
    CALL hive.dlogs('<no-context>', 'Entering drop_triggers');
        -- register insert trigger
    EXECUTE format(
    ...
    CALL hive.dlogs('<no-context>', 'Exiting drop_triggers');
