...
BEGIN
        ...
        operations AS (
                SELECT
                       hive.operation_id_to_block_num_wrapper(ho.id) as block_num
                     , ho.trx_in_block
                     , ARRAY_AGG(ho.body_binary ORDER BY op_pos ASC) bodies
                FROM hive.operations_view ho
                WHERE
                    hive.operation_id_to_type_id_wrapper(ho.id) <= (SELECT ot.id FROM hive.operation_types ot WHERE ot.is_virtual = FALSE ORDER BY ot.id DESC LIMIT 1)
                    AND hive.operation_id_to_block_num_wrapper(ho.id) BETWEEN _block_num_start AND ( _block_num_start + _block_count - 1 )
                GROUP BY hive.operation_id_to_block_num_wrapper(ho.id), ho.trx_in_block
                ORDER BY hive.operation_id_to_block_num_wrapper(ho.id) ASC, trx_in_block ASC
        ),
