...
# DONE mtlk  -- generally can move rewind_undo_state functionality where it was
# TODO remove replay_blocks out of class
# TODO - erase nodes from fork db after pop_block ?
# DONE - eliminate removing undo_all at the beginning
# DONE taking data from hive.block_view , perhaps from hive.operations, hive.transactions - views ?
# DONE long long balance;
...
# PHASED OUT rename IBlockProvider to Iblock_log
# PHASED OUT test na start_fork - DIP 
...
# PHASED OUT make it truly  switchable with classic version - command line param = switching from  trans to non trans, -> eliminate ?
...
#    ninja extension.hive_fork_manager  \
    ninja consp_driver tests/unit/all query_supervisor hived extension.hive_fork_manager && sudo ninja install && sudo chown $USER:$USER .ninja_* && ctest -R keyauth --output-on-failure && ctest -R curr --output-on-failure 
    EXIT_STATUS=$?
