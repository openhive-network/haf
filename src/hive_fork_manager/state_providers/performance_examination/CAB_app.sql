...
INSERT INTO CAB_app.app_status
(continue_processing, last_processed_block)
VALUES
(True, 0)
;

CREATE TABLE IF NOT EXISTS CAB_app.dummy_table_to_make_it_forking_application
(
) 
INHERITS (hive.CAB_app)
;
END
$$
;
...
$$
BEGIN
  UPDATE CAB_app.app_status SET last_processed_block = _lastBlock;
END
$$
;
...
BEGIN
  ...
  FOR b IN _from .. _to BY _step LOOP
    ...
    EXIT WHEN NOT CAB_app.continueProcessing();
...
BEGIN
  ...
  CREATE TABLE if not exists  CAB_app.LOG_TABLE (first_block INT, last_block INT, time INTERVAL);
  ...
  IF NOT hive.app_context_exists(_appContext) THEN
    ...
    PERFORM hive.app_state_provider_import('CAB', _appContext);
  ...
  WHILE CAB_app.continueProcessing() AND (_maxBlockLimit = 0 OR __last_block < _maxBlockLimit) LOOP
    ...
    ELSE
      ...
      IF __next_block_range.first_block != __next_block_range.last_block THEN
        ...
        INSERT INTO CAB_app.LOG_TABLE VALUES(__next_block_range.first_block, __next_block_range.last_block, temp_interval);
