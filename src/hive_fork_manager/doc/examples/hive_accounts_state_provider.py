...
def prepare_application_data( db_connection ):
        ...
        if exist[ 0 ] == False:
            db_connection.execute( text("SELECT hive.app_create_context( '{}', _schema=>'applications', _is_forking => TRUE )".format( APPLICATION_CONTEXT )) )
        ...
        db_connection.execute( text("SELECT hive.app_state_provider_import( 'ACCOUNTS', '{}' )".format( APPLICATION_CONTEXT )) );
...
def main_loop( db_connection ):
    ...
    while True:
        ...
        with db_connection.begin():
            ...
            accounts = db_connection.execute( text("SELECT * FROM hafd.{}_accounts ORDER BY id DESC LIMIT 1".format( APPLICATION_CONTEXT )) ).fetchall()
            ...
            if ( last_block - first_block ) > 100:
                ...
                db_connection.execute( text("SELECT hive.app_state_providers_update( {}, {}, '{}' )".format( first_block, last_block, APPLICATION_CONTEXT )) )
                ...
                db_connection.execute( text("SELECT hive.app_context_attach( '{}' )".format( APPLICATION_CONTEXT )) )
                continue

            # process the first block in range - one commit after each block
            db_connection.execute( text("SELECT hive.app_state_providers_update( {}, {}, '{}' )".format( first_block, first_block, APPLICATION_CONTEXT )) )
