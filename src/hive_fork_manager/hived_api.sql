...
CREATE OR REPLACE FUNCTION hive.push_block(
    ...
    , _operations hafd.operations_input[]
...
BEGIN
    ...
    INSERT INTO hafd.operations_reversible(id, trx_in_block, op_type_id, op_pos, body_value, custom_json_type_id, fork_id)
      SELECT id, trx_in_block, op_type_id, op_pos,
             (body_binary::hafd.operation::jsonb)->'value',
             custom_json_type_id, __fork_id
      FROM unnest( _operations );
...
-- Compatibility overloads: accept hafd.operations[] (with pre-computed body_value jsonb).
-- Used by SQL tests and any callers that already have JSONB operation data.
CREATE OR REPLACE FUNCTION hive.push_block(
      _block hafd.blocks
    , _transactions hafd.transactions[]
    , _signatures hafd.transactions_multisig[]
