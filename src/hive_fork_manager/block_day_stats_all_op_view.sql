...
CROSS JOIN (SELECT ot.id AS op_type_id FROM hive_data.operation_types ot) t
...
$$
,
$$ SELECT ot.name FROM hive_data.operation_types ot ORDER BY 1 $$ -- Must match the order with interface specification
) AS ct(block_day INT,
...
declined_voting_rights_operation INT,
