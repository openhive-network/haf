...
WITH 
consistent_block AS
(SELECT COALESCE(hid.consistent_block, 0) AS consistent_block FROM hive.get_irr_data() hid LIMIT 1)
,forks AS
(
...
(
    ...
    FROM (
        ...
        JOIN (
            ...
            WHERE hbr.num > ( SELECT COALESCE( hid.consistent_block, 0 ) FROM hive.get_irr_data() hid )
            GROUP by hbr.num
...
FROM (
    ...
    (
         ...
         WHERE rb.num > ( SELECT COALESCE( hid.consistent_block, 0 ) FROM hive.get_irr_data() hid )
         GROUP BY rb.num
