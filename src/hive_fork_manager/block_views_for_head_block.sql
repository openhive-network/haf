...
WHERE
    ...
    (
        ...
        AND ha.block_id = (
            SELECT hb.block_id
            FROM hafd.blocks hb
            WHERE hb.block_num = hafd.block_id_to_num(ha.block_id)
        ...
        AND ha.block_id = (
            ...
            WHERE ha2.id = ha.id
              ...
              AND ha2.block_id = (
                  SELECT hb.block_id
                  FROM hafd.blocks hb
                  WHERE hb.block_num = hafd.block_id_to_num(ha2.block_id)
                  ORDER BY hb.block_id DESC
                  LIMIT 1
              )
...
OR (
    ...
    htm.block_id = (
        SELECT hb.block_id
        FROM hafd.blocks hb
        WHERE hb.block_num = hafd.block_id_to_num(htm.block_id)
        ORDER BY hb.block_id DESC
        LIMIT 1
    )
);
