...
WITH 
consistent_block AS
(SELECT COALESCE(COALESCE(hid.consistent_block, 0),0) AS consistent_block FROM hive.irreversible_data hid LIMIT 1)
,forks AS
(
...
(
    ...
    FROM (
        ...
        JOIN (
            ...
            WHERE hbr.num > ( SELECT COALESCE(COALESCE( hid.consistent_block, 0 ),0) FROM hive.irreversible_data hid )
            GROUP by hbr.num
...
FROM (
    ...
    (
         ...
         WHERE rb.num > ( SELECT COALESCE(COALESCE( hid.consistent_block, 0 ), 0) FROM hive.irreversible_data hid )
         GROUP BY rb.num
