...
BEGIN
    ...
    -- The caller (end_massive_sync / set_irreversible) holds EXCLUSIVE lock on
    -- hafd.contexts_attachment, which blocks any concurrent squash_and_get_state
    -- (that holds ROW SHARE on the same table). This ensures no application can
    -- modify contexts.events_id while we read MIN(events_id) and delete events.
