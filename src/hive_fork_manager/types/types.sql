...
CREATE TYPE hive.votable_asset_info_v1 AS (
  max_accepted_payout hafd.share_type,
...
CREATE TYPE hive.allowed_vote_asset AS (
  asset_symbol_type hafd.asset_symbol,
...
CREATE TYPE hive.comment_operation AS (
  parent_author hafd.account_name_type,
  parent_permlink hafd.permlink,
  author hafd.account_name_type,
  permlink hafd.permlink,
  title hafd.comment_title,
  body text,
  json_metadata TEXT
);
...
CREATE TYPE hive.account_auth AS (
  name hafd.account_name_type,
  weight hafd.weight_type
);

DROP TYPE IF EXISTS hive.key_auth CASCADE;
CREATE TYPE hive.key_auth AS (
  public_key hafd.public_key_type,
  weight hafd.weight_type
);
...
CREATE TYPE hive.account_create_operation AS (
  fee hive.asset,
  creator hafd.account_name_type,
  new_account_name hafd.account_name_type,
  owner hive.authority,
  active hive.authority,
  posting hive.authority,
  memo_key hafd.public_key_type,
  json_metadata TEXT
);
...
CREATE TYPE hive.change_recovery_account_operation AS (
  account_to_recover hafd.account_name_type,
  new_recovery_account hafd.account_name_type,
  extensions hive.extensions_type
);
...
CREATE TYPE hive.custom_binary_operation AS (
  required_owner_auths hafd.account_name_type[],
  required_active_auths hafd.account_name_type[],
  required_posting_auths hafd.account_name_type[],
  required_auths hive.authority[],
  id hafd.custom_id_type,
  data bytea
);

DROP TYPE IF EXISTS hive.custom_json_operation CASCADE;
CREATE TYPE hive.custom_json_operation AS (
  required_auths hafd.account_name_type[],
  required_posting_auths hafd.account_name_type[],
  id hafd.custom_id_type,
  json TEXT
);
...
CREATE TYPE hive.delegate_vesting_shares_operation AS (
  delegator hafd.account_name_type,
  delegatee hafd.account_name_type,
...
CREATE TYPE hive.escrow_approve_operation AS (
  "from" hafd.account_name_type,
  "to" hafd.account_name_type,
  agent hafd.account_name_type,
  who hafd.account_name_type,
...
CREATE TYPE hive.escrow_release_operation AS (
  ...
  agent hafd.account_name_type,
  who hafd.account_name_type,
  receiver hafd.account_name_type,
  escrow_id int8, -- uint32_t: 4 byte, but unsigned (int8)
...
CREATE TYPE hive.feed_publish_operation AS (
  publisher hafd.account_name_type,
...
CREATE TYPE hive.pow2_input AS (
  worker_account hafd.account_name_type,
  prev_block hafd.block_id_type,
  nonce NUMERIC
);
...
CREATE TYPE hive.request_account_recovery_operation AS (
  recovery_account hafd.account_name_type,
  account_to_recover hafd.account_name_type,
  new_owner_authority hive.authority,
  extensions hive.extensions_type
);
...
CREATE TYPE hive.reset_account_operation AS (
  reset_account hafd.account_name_type,
  account_to_reset hafd.account_name_type,
  new_owner_authority hive.authority
);
...
CREATE TYPE hive.set_reset_account_operation AS (
  account hafd.account_name_type,
  current_reset_account hafd.account_name_type,
  reset_account hafd.account_name_type
);
...
CREATE TYPE hive.set_withdraw_vesting_route_operation AS (
  from_account hafd.account_name_type,
  to_account hafd.account_name_type,
...
CREATE TYPE hive.witness_update_operation AS (
  owner hafd.account_name_type,
  url hafd.permlink,
  block_signing_key hafd.public_key_type,
  props hive.legacy_chain_properties,
  fee hive.asset
);
...
CREATE TYPE hive.changed_recovery_account_operation AS (
  account hafd.account_name_type,
  old_recovery_account hafd.account_name_type,
  new_recovery_account hafd.account_name_type
);
...
CREATE TYPE hive.comment_benefactor_reward_operation AS (
  benefactor hafd.account_name_type,
  author hafd.account_name_type,
  permlink hafd.permlink,
  hbd_payout hive.asset,
  hive_payout hive.asset,
...
CREATE TYPE hive.comment_reward_operation AS (
  ...
  permlink hafd.permlink,
  payout hive.asset,
  author_rewards hafd.share_type,
...
CREATE TYPE hive.curation_reward_operation AS (
  curator hafd.account_name_type,
  reward hive.asset,
  author hafd.account_name_type,
  permlink hafd.permlink,
  payout_must_be_claimed boolean
...
CREATE TYPE hive.hardfork_hive_operation AS (
  account hafd.account_name_type,
  treasury hafd.account_name_type,
  other_affected_accounts hafd.account_name_type[],
