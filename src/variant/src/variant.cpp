...
typedef struct VariantCache
{
	...
	const char						*formatted_name;	/* Formatted type string. Only set when IOfunc is output/send */
} VariantCache;
...
{
	VariantInt			vi =(VariantInt)(palloc0(sizeof(*vi)));
...
variant_typmod_in(PG_FUNCTION_ARGS)
{
	...
	/* Keep cruft localized to just here */
	{
		...
		const char						*cmd = "SELECT variant_typmod, variant_enabled FROM variant._registered WHERE lower(variant_name) = lower($1)";
		...
		if ( SPI_processed > 1 )
			ereport(ERROR,
				( errmsg( "Got %lu records for variant.variant(%s)", SPI_processed, inputCString ),
...
variant_get_variant_name(int typmod, Oid org_typid, bool ignore_storage)
{
	...
	if ( SPI_processed > 1 )
		ereport(ERROR,
			( errmsg( "Got %lu records for variant typmod %i", SPI_processed, typmod ),
				errhint( "This means _variant._registered is corrupted" )
			)
		);
	if ( SPI_processed < 1 )
...
variant_cmp_int(FunctionCallInfo fcinfo)
{
	...
	/* TODO: cache this */
	{
		...
		if( (ret = SPI_execute_with_args(
						cmd, 2, types, values, (const char*)(nulls),
						true, /* read-only */
						0 /* count */
					)) != SPI_OK_SELECT )
			elog( ERROR, "SPI_execute_with_args returned %s", SPI_result_code_string(ret));
...
make_variant_int(Variant v, FunctionCallInfo fcinfo, IOFuncSelector func)
{
	...
	vi =(VariantInt)(palloc0(sizeof(VariantDataInt)));
	...
	else /* Fixed size, pass by reference */
	{
		...
		ptr =(Pointer)(palloc0(data_length));
...
make_variant(VariantInt vi, FunctionCallInfo fcinfo, IOFuncSelector func)
{
	...
	v =(Variant)(palloc0(variant_length));
