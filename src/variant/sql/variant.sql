...
CREATE OR REPLACE FUNCTION _variant.variant_%1$s(variant.variant, variant.variant)
  RETURNS boolean LANGUAGE c IMMUTABLE STRICT AS '$libdir//libvariant-@GIT_REVISION@.so', 'variant_%1$s';
  $$
  , op
) )
...
BEGIN
  PERFORM _variant.exec(
    format(
      ...
      , typmod int
      , explicit boolean
    ) RETURNS variant.variant LANGUAGE c IMMUTABLE AS '$libdir//libvariant-@GIT_REVISION@.so', 'variant_cast_in'
      $sql$
...
BEGIN
  PERFORM _variant.exec(
    format(
      ...
      v variant.variant
    ) RETURNS %1s LANGUAGE c IMMUTABLE AS '$libdir//libvariant-@GIT_REVISION@.so', 'variant_cast_out'
      $sql$
      , v_function_name
      , p_target
    )
...
CREATE VIEW _variant.stored AS
  SELECT atttypmod AS variant_typmod, quote_ident(attname) AS column_name, a.attrelid, a.atttypid
    FROM pg_attribute a
