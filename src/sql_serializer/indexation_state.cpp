...
void
indexation_state::on_pre_reindex( cached_data_t& cached_data, int last_block_num, uint32_t stop_process_at ) {
  ...
  update_state( INDEXATION::REINDEX, cached_data, last_block_num, stop_process_at );
}
...
uint32_t
indexation_state::expected_number_of_blocks_to_sync( uint32_t stop_process_at, uint32_t last_block_num ) const {
  uint32_t _blocks = 0;

  if( stop_process_at )
    _blocks = stop_process_at - last_block_num;
  else
    _blocks = ( fc::time_point::now() - _chain_db.head_block_time() ).to_seconds() / 3;

  ilog("Information about disabling/enabling contraints/indexes - now: ${now} head: ${head} stop_process_at: ${stop_process_at}",
  ("now", fc::time_point::now())("head", _chain_db.head_block_time())(stop_process_at));
  return _blocks;
}
...
indexation_state::update_state(
  ...
  , uint32_t last_block_num, uint32_t stop_process_at
) {
  ...
  switch ( state ) {
    ...
    case INDEXATION::REINDEX:
      ...
      ilog(" last block number: ${last_block_num} stop process at: ${stop_process_at}",(last_block_num)(stop_process_at));
