...
namespace hive::plugins::sql_serializer {
  ...
  struct hive_account_operations
    ...
    struct data2sql_tuple : public data2_sql_tuple_base
      ...
      // Used for live sync (push_block) - generates format matching hafd.account_operations_type
      // Format: (account_id, transacting_account_id, account_op_seq_no, operation_id, op_type_id)
      std::string operator()(typename container_t::const_reference data) const
      {
        return std::to_string(data.account_id) + ',' + std::to_string(data.transacting_account_id) + ',' +
        std::to_string(data.operation_seq_no) + ',' + std::to_string(data.operation_id) + ',' + std::to_string(data.op_type_id);
      }
      };
    };
