...
# Use RUN --mount instead of COPY to support variable substitution in source paths.
RUN --mount=from=pgcron-builder,source=/pgcron,target=/tmp/pgcron \
    cp /tmp/pgcron/usr/lib/postgresql/${POSTGRES_VERSION}/lib/pg_cron.so /usr/lib/postgresql/${POSTGRES_VERSION}/lib/ && \
    cp /tmp/pgcron/usr/share/postgresql/${POSTGRES_VERSION}/extension/pg_cron* /usr/share/postgresql/${POSTGRES_VERSION}/extension/
...
COPY --from=build "${HAF_SOURCE_DIR}/docker/cron_jobs.sql" /docker-entrypoint-always-initdb.d/cron_jobs.dat
