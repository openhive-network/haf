...
from generate_block_log_with_varied_signature_types import (
    ACCOUNT_NAMES,
...
def test_block_log_with_several_type_of_signatures(replayed_node: tt.InitNode, account: str) -> None:
    ...
    assert node.api.database.find_accounts(accounts=[account]).accounts[0].balance >= tt.Asset.Test(110)
    assert node.api.database.find_accounts(accounts=[account]).accounts[0].hbd_balance == TBD_PER_ACCOUNT
    ...
    assert node.api.database.find_accounts(accounts=[account]).accounts[0].vesting_shares >= tt.Asset.Vest(104000)
    assert (
        node.api.database.find_accounts(accounts=[account]).accounts[0].received_vesting_shares
        == DELEGATION_PER_ACCOUNT
    )
