...
def test_memo_scanner_fork(world_with_witnesses_and_database):
    logger.info(f'Start test_memo_scanner_fork')
    ...
    prepared_trx = beta_wallet.api.transfer('initminer', 'initminer', Asset.Test(1000), f'dummy transfer operation', broadcast=False)

    # WHEN
    make_fork(
        world,
        at_block = START_TEST_BLOCK,
        fork_chain_trxs = [prepared_trx],
    )
    wait_for_application_context(session)
  
    memos = session.execute( "SELECT * FROM memo_scanner.memos").all()
    logger.info( "memos: {}\n".format( memos ) )
    assert len(memos) == 1 # assert there is one trx from fork
