...
CREATE OR REPLACE PROCEDURE test.scenario2_prepare(IN _time_shift INTERVAL = '3 hrs'::interval)
LANGUAGE 'plpgsql'
AS
$$
BEGIN
...
CREATE OR REPLACE PROCEDURE test.scenario2_verify(IN _time_shift INTERVAL = '3 hrs'::interval)
LANGUAGE 'plpgsql'
AS
$$
BEGIN
    -- context are not detached because haf restart override their last_active_at
   SET ROLE test_app_owner;

   PERFORM test.verify_is_attached_context('active_app', TRUE);
   PERFORM test.verify_is_attached_context('dead_app1', TRUE);
END
$$
