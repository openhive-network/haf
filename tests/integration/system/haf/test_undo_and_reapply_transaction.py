...
def test_undo_and_reapply_transaction(world_with_witnesses_and_database):
    ...
    # THEN
    trxs = session.query(Transactions).filter(Transactions.block_num == START_TEST_BLOCK + 1).all()
    assert len(trxs) == 0
    trxs_rev = session.query(TransactionsReversible).filter(TransactionsReversible.block_num == START_TEST_BLOCK + 1).all()
    assert len(trxs_rev) == 2

    wait_for_irreversible_progress(node_under_test, after_fork_block)

    trx = session.query(Transactions).filter(Transactions.block_num == START_TEST_BLOCK + 1).one()
