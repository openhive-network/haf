...
def pg_restore_from_dump_file_only(target_db_name: str, tmp_path: Path) -> None:
    dump_file_path = tmp_path / DUMP_FILENAME
    shell(f"pg_restore --section=pre-data -Fc -d {target_db_name} {dump_file_path}")
    shell(f"pg_restore --section=data -Fc --disable-triggers -d {target_db_name} {dump_file_path}")
