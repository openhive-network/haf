...
$BODY$
BEGIN
    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context_a' AND hca.is_attached = FALSE ), 'Attach flag is still set context_a';
    ...
    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context_b' AND hca.is_attached = FALSE ), 'Attach flag is still set context_b';
    ...
    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context_c' AND hca.is_attached = FALSE ), 'Attach flag is still set context_c';
