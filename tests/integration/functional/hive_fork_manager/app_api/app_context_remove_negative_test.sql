...
BEGIN
    ...
    ASSERT EXISTS ( SELECT FROM information_schema.tables WHERE table_schema='hive_data' AND table_name  = 'shadow_a_table1' ), 'shadow table exists';
    ...
    ASSERT EXISTS ( SELECT FROM hive_data.registered_tables WHERE origin_table_schema='a' AND origin_table_name='table1' AND shadow_table_name='shadow_a_table1' ), 'entry about';
    ASSERT EXISTS ( SELECT FROM pg_trigger WHERE tgname='hive_data.insert_trigger_a_table1'), 'Insert trigger not dropped';
    ASSERT EXISTS ( SELECT * FROM pg_proc WHERE proname = 'on_insert_a_table1'), 'Insert trigger function not dropped';
    ASSERT EXISTS ( SELECT FROM pg_trigger WHERE tgname='hive_data.delete_trigger_a_table1' ), 'Delete trigger not dropped';
    ASSERT EXISTS ( SELECT * FROM pg_proc WHERE proname = 'on_delete_a_table1') ,'Delete trigger function not dropped';
    ASSERT EXISTS ( SELECT FROM pg_trigger WHERE tgname='hive_data.update_trigger_a_table1' ), 'Update trigger not dropped';
