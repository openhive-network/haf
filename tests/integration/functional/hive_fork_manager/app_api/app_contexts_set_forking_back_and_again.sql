...
BEGIN
    ...
    CREATE SCHEMA B;
    PERFORM hive.app_create_context( 'context_b', _schema => 'b', _is_forking =>FALSE );
    CREATE TABLE B.table1( id INT) INHERITS( b.context_b );
...
BEGIN
    ...
    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive_data.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context' AND hca.is_attached = TRUE ), 'Attach flag is still set';
    ASSERT ( SELECT current_block_num FROM hive_data.contexts WHERE name='context' ) = 2, 'Wrong current_block_num';
    ASSERT ( SELECT is_forking FROM hive_data.contexts WHERE name='context' ) = TRUE, 'context is is still marked as non-forking';

    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive_data.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context_b' AND hca.is_attached = TRUE ), 'b) Attach flag is still set';
    ASSERT ( SELECT current_block_num FROM hive_data.contexts WHERE name='context_b' ) = 2, 'b) Wrong current_block_num';
    ASSERT ( SELECT is_forking FROM hive_data.contexts WHERE name='context_b' ) = TRUE, 'b) context is is still marked as non-forking';
