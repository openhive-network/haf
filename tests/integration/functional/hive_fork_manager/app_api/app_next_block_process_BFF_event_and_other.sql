...
BEGIN
    ...
    ASSERT ( SELECT hc.events_id FROM hafd.contexts hc WHERE hc.name = 'context' ) = 5, 'The context is not on BFF(2) event';
...
BEGIN
    ...
    -- from already processed events: DELETE operation on hafd.events_queue takes eid=5 as an upper bound, but after the context move
    -- its ctx may back to eid=3, and DELETE will fail what stops the hived process
    ASSERT ( SELECT hc.events_id FROM hafd.contexts hc WHERE hc.name = 'context' ) = 6, 'Wrong events_id after move context in new fork';
END
$BODY$
;
