...
BEGIN
    ...
    SELECT ((stg).stage::hive_data.application_stage).* FROM hive.get_current_stage( ARRAY[ 'alice'] ) stg INTO __current_stage;
    ASSERT __context = 'alice' , 'ctx != alice' ;
    ASSERT __current_stage = hive_data.live_stage(), 'alice stage != live';

    -- alice1 live
    SELECT (stg).context FROM hive.get_current_stage( ARRAY[ 'alice1'] ) stg INTO __context;
    SELECT ((stg).stage::hive_data.application_stage).* FROM hive.get_current_stage( ARRAY[ 'alice1'] ) stg INTO __current_stage;
    ASSERT __context = 'alice1' , 'ctx != alice1' ;
    ASSERT __current_stage = hive_data.live_stage(), 'alice1 stage  != live';

    -- alice2 stage2
    SELECT (stg).context FROM hive.get_current_stage( ARRAY[ 'alice2'] ) stg INTO __context;
    SELECT ((stg).stage::hive_data.application_stage).* FROM hive.get_current_stage( ARRAY[ 'alice2'] ) stg INTO __current_stage;
    ASSERT __context = 'alice2' , 'ctx != alice2' ;
    ASSERT __current_stage = hive_data.live_stage(), 'alice2 stage  != live';


    SELECT ((stg).stage::hive_data.application_stage).* FROM hive.get_current_stage( ARRAY[ 'alice', 'alice1', 'alice2' ] ) stg
    WHERE (stg).context = 'alice'  INTO __current_stage;
    ASSERT __current_stage = hive_data.live_stage(), 'alice stage != (''stage1'',10 ,10 )';
    ...
    ASSERT __current_stage = hive_data.live_stage(), 'alice2 stage  != (''stage2'',40 ,100 )';
END;
$BODY$;