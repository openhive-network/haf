...
BEGIN
    ...
    PERFORM hive.app_state_provider_import( 'c_a_b_s_t', 'context' );
...
BEGIN
    ...
    PERFORM hive.update_state_provider_c_a_b_s_t( 1, 5, 'context' );
END;
$BODY$
;
...
DECLARE
BEGIN
    ASSERT 6 = (SELECT * FROM hive.consensus_state_provider_get_expected_block_num('context')), 'consensus_state_provider_get_expected_block_num should return 6';
    ASSERT EXISTS ( SELECT * FROM hive.context_c_a_b_s_t WHERE account = 'initminer' AND balance = 4000), 'Incorrect balance of initminer';
    ASSERT EXISTS ( SELECT * FROM hive.context_c_a_b_s_t WHERE account = 'miners' AND balance = 0),'Incorrect balance of miners';
    ...
    ASSERT 4 = ( SELECT COUNT(*) FROM hive.context_c_a_b_s_t), 'Incorrect number of accounts';
