...
BEGIN
FOR _row in
  SELECT (ROW(f.table_name, f.table_schema_hash, f.columns_hash, f.constraints_hash, f.indexes_hash)::text) FROM hive.calculate_schema_hash('hive') AS f
  EXCEPT SELECT unnest(ARRAY[
    ...
    '(contexts,626adada-77cc-1795-4acf-1015632f4cfa,79bce9e9-459c-e9a1-397e-2a0ebf529c7f,4a82cf7a-fd28-61ec-f852-e591c0690ad0,8672562f-b341-b429-c70d-0d9a00dd18d7)'
    ])
LOOP
