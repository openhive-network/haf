-- Load test utilities
\ir ../test_tools.sql

CREATE OR REPLACE PROCEDURE haf_admin_test_given()
        LANGUAGE 'plpgsql'
AS
$BODY$
BEGIN
    INSERT INTO hafd.fork( id, block_num, time_of_fork)
    VALUES ( 2, 6, '2020-06-22 19:10:25-07'::timestamp ),
           ( 3, 7, '2020-06-22 19:10:25-07'::timestamp );

    -- Create irreversible blocks 1-8 (producer_id=1 for this test)
    PERFORM test.create_blocks(1, 8, 1);

    -- Reversible blocks for fork 1 (blocks 4-9)
    PERFORM test.create_blocks_reversible(4, 9, 1, 1);

    -- Reversible blocks for fork 2 (blocks 7-9)
    PERFORM test.create_blocks_reversible(7, 9, 2, 1);

    -- Reversible blocks for fork 3 (blocks 8-10)
    PERFORM test.create_blocks_reversible(8, 10, 3, 1);

    INSERT INTO hafd.accounts
    VALUES
           ( 1, 'alice1', 1 )
         , ( 2, 'alice2', 2 )
         , ( 3, 'alice3', 3 )
         , ( 4, 'alice4', 4 )
    ;

    INSERT INTO hafd.accounts_reversible
    VALUES
           ( 4, 'alice41', 4, 1 )
         , ( 5, 'alice51', 5, 1 )
         , ( 6, 'alice61', 6, 1 )
         , ( 7, 'alice71', 7, 1 ) -- must be overriden by fork 2
         , ( 8, 'bob71', 7, 1 )   -- must be overriden by fork 2
         , ( 9, 'alice81', 8, 1 ) -- must be overriden by fork 2
         , ( 7, 'alice72', 7, 2 )
         , ( 8, 'bob72', 7, 2 )
         , ( 9, 'alice82', 8, 2 )
         , ( 10, 'alice92', 9, 2 )
         , ( 9, 'alice83', 8, 3 )
         , ( 10, 'alice93', 9, 3 )
         , ( 11, 'alice103', 10, 3 )
    ;
END;
$BODY$
;

CREATE OR REPLACE PROCEDURE haf_admin_test_when()
LANGUAGE 'plpgsql'
AS
$BODY$
BEGIN
    PERFORM hive.copy_accounts_to_irreversible( 4, 8 );
END
$BODY$
;

CREATE OR REPLACE PROCEDURE haf_admin_test_then()
        LANGUAGE 'plpgsql'
AS
$BODY$
BEGIN
    ASSERT NOT EXISTS (
        SELECT * FROM hafd.accounts WHERE id >= 0
        EXCEPT SELECT * FROM ( VALUES
                   (  1, 'alice1' , 1)
                 , (  2, 'alice2' , 2)
                 , (  3, 'alice3' , 3)
                 , (  4, 'alice4' , 4)
                 , (  5, 'alice51', 5 )
                 , (  6, 'alice61', 6 )
                 , (  7, 'alice72', 7 )
                 , (  8, 'bob72'  , 7)
                 , (  9, 'alice83', 8 )
                 ) as pattern
    ) , 'Unexpected rows in the view';

    ASSERT NOT EXISTS (
        SELECT * FROM ( VALUES
                   (  1, 'alice1' , 1)
                 , (  2, 'alice2' , 2)
                 , (  3, 'alice3' , 3)
                 , (  4, 'alice4' , 4)
                 , (  5, 'alice51', 5 )
                 , (  6, 'alice61', 6 )
                 , (  7, 'alice72', 7 )
                 , (  8, 'bob72'  , 7)
                 , (  9, 'alice83', 8 )
        ) as pattern
        EXCEPT SELECT * FROM hafd.accounts WHERE id >= 0
    ) , 'Unexpected rows in the view2';

END
$BODY$
;




