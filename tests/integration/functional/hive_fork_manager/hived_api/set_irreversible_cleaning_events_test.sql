...
BEGIN
    ...
    ASSERT ( SELECT hid.consistent_block FROM hafd.hive_state hid ) = 4 , 'Wrong consisten irreversible block';
    ASSERT EXISTS ( SELECT * FROM hafd.events_queue WHERE event = 'NEW_IRREVERSIBLE' AND block_num=4 ), 'No NEW_IRREVERSIBLE event 4';
