...
raise notice 'hive.blocks=%',
(
    select json_agg(t)
    FROM (
            SELECT num, hash, prev
            FROM hive.blocks
        ) t
);
...
raise notice 'PUSHED_block9:=%',
(
    select json_agg(t) FROM
    ( SELECT num, hash FROM hive.blocks_view JOIN hive.accounts ON  id = producer_account_id ORDER BY num ASC )
        t
);
...
PERFORM hive.push_block(
        (10, '\x0000000a3f173d1264fe6654497eba5a508ba793', '\x00000009d361e47148af8fe7598c4f2db74237ed', '2016-03-24 16:06:00', 3, '\x0000000000000000000000000000000000000000', NULL, '\x1f6ac53a8bb6ca885e988baafb1363b98e8807f62e6256462269b7288c568010096402d9bd2d8a69549568477f79570e8a41474daee2b7d29c623a0b5649081417', 'STM8GC13uCZbP44HzMLV6zPZGwVQ8Nt4Kji8PapsPiNq1BK153XTX', 0, 1000, 1000000, 20000, 30000, 30000, 0, 0)
        , NULL
        , NULL
        , NULL
        , NULL
...
ASSERT '\x0000000a3f173d1264fe6654497eba5a508ba793'::bytea = ( SELECT hash FROM hive.context_blocks_view WHERE num = 10 ), 'Unexpect hash of block 10 2nd';
INSERT INTO A.table1(id) VALUES( 10 );

raise notice 'PUSHED_QUERY_FROM_C_block10:=%',
(
    select json_agg(t) FROM
    (SELECT * FROM hive.blocks_view JOIN hive.accounts ON  id = producer_account_id WHERE num >= 10 and num <= 10 ORDER BY num ASC)
    t
);
