...
BEGIN
    ...
    -- Forking columns should not exist in contexts table
    ASSERT NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema='hafd' AND table_name='contexts' AND column_name='is_forking'), 'is_forking column should not exist';
    ASSERT NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema='hafd' AND table_name='contexts' AND column_name='fork_id'), 'fork_id column should not exist';
END;
$BODY$
;
