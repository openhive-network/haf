...
BEGIN
    ...
    ASSERT ( SELECT application_fork FROM hafd.contexts_log WHERE id = _last_entry_id ) = 0 , 'Wrong context app fork';
    ASSERT ( SELECT head_fork_id FROM hafd.contexts_log WHERE id = _last_entry_id ) = 0 , 'Wrong context head fork';
