...
BEGIN
    ...
    CREATE TABLE a.table2( id INTEGER NOT NULL, smth hive.ctext NOT NULL ) INHERITS( a.context2 );
    PERFORM hive.context_next_block( 'context2' );
    INSERT INTO a.table2( id, smth ) VALUES( 123, 'blabla' );
    PERFORM hive.context_next_block( 'context2' );

    TRUNCATE hive.shadow_a_table1; --to do not revert context inserts
    UPDATE a.table1 SET id=321;
    PERFORM hive.context_next_block( 'context' );
    DELETE FROM  a.table1 WHERE id=321;

    TRUNCATE hive.shadow_a_table2; --to do not revert context inserts
...
BEGIN
    ...
    ASSERT ( SELECT COUNT(*) FROM hive.shadow_a_table2 ) != 0, 'context2 shadow table was empty';
