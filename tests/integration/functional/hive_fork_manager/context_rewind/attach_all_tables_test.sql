...
$BODY$
BEGIN
    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context' AND hca.is_attached = TRUE ), 'Attach flag is still not set';
    ...
    ASSERT EXISTS ( SELECT * FROM hive_data.contexts hc JOIN hive.contexts_attachment hca ON hca.context_id=hc.id WHERE hc.name='context2' AND hca.is_attached = FALSE ), 'Attach flag is still set';
