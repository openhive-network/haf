...
-- NOTES:
-- 1. The test logic and assertions remain completely unchanged
-- 2. Only the data setup (haf_admin_test_given) was refactored
-- 3. We kept one custom INSERT for block 10 as it has special producer requirements
-- 4. Test behavior is identical - same blocks are created with same values
-- 5. Hash generation in test_tools uses a formula that matches the original pattern
