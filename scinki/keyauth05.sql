...
BEGIN
    ...
    raise notice 'mtk hereare';

    __array := json_extract_path( CAST( _account_operation as json ), 'value', _entity,'key_auths');

    raise notice 'mtk juztu';
...
BEGIN
    ...
    __array := hive.get_key_from_entity(_account_operation, 'owner');
  raise notice 'mtk przeszlo';

  raise notice '__array is %', __array;

   __retarr_temp :=  __array;

   __retarr := __retarr_temp;



    __array := hive.get_key_from_entity(_account_operation, 'active');
   __retarr_temp :=  __array;
   __retarr :=  __retarr || __retarr_temp;

    __array := hive.get_key_from_entity(_account_operation, 'posting');
   __retarr_temp :=  __array;
   __retarr :=  __retarr || __retarr_temp;
